...
public:
	void test()
	{
		...
		if (i%100 > 66)
		{
			sprintf(reqBuff, "http://127.0.0.1:4567/add?a=%d&b=1", i);
		...
		{
			sprintf(reqBuff, "http://127.0.0.1:4567/sub?a=%d&b=1", i);
			this->get(reqBuff, [this](HttpClientC* pHttpClient) {
				if (pHttpClient)
				{
					CELLLog_Info("recv server msg. len=%s %d| %s",
			...
			this->post("http://127.0.0.1:4567/jsonTest", reqBuff, [this](HttpClientC* pHttpClient) {
				if (pHttpClient)
				{
					CELLLog_Info("recv server msg. len=%s %d| %s",
						pHttpClient->header_getStr("Content-Length", "?"),
